            ...
            __newindex = function(self, k, v)
                    ...
                    if objchildren[parent] then
                        if listobjs[parent] then
                            for i, object in next, objchildren[parent] do
                                ...
                                newpos = Vector2.new(utility:Round(newpos.X), utility:Round(newpos.Y))
                ...
                if k == "Size" then
                    ...
                    if typeof(v) == "Vector2" then
                        v = Vector2.new(utility:Round(v.X), utility:Round(v.Y))
                    end
                end
...
function utility:colortotable(color)
    local r, g, b = self:Round(color.R * 255),  self:Round(color.G * 255), self:Round(color.B * 255)
    return {r, g, b}
end

function utility:ToColor(...)
    local Args = {...}
    if typeof(Args[1]) == "table" then
        return Color3.fromRGB(unpack(Args[1]))
...
function utility:Round(number, decimals)
    decimals = typeof(decimals) == "number" and decimals >= 0 and decimals or 0
    local float = 1
...
function utility:UnpackRBG(color)
    local r = color.R * 255
    local g = color.G * 255
    local b = color.B * 255
...
function utility.changecolor(color, number)
    local r, g, b = utility:UnpackRBG(color)
...
function library.createcolorpicker(default, defaultalpha, parent, count, flag, callback)
    ...
    local text = utility:Create("Text", {
        Text = string.format("%s, %s, %s", utility:Round(default.R * 255), utility:Round(default.G * 255), utility:Round(default.B * 255)),
        Font = Drawing.Fonts.Plex,
        Size = 13,
    ...
    local function set(color, a, nopos)
        ...
        if hsv ~= oldcolor or alpha ~= oldalpha then
            ...
            text.Text = string.format("%s, %s, %s", utility:Round(hsv.R * 255), utility:Round(hsv.G * 255), utility:Round(hsv.B * 255))
